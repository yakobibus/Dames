...
namespace spc_dames
{
	Ardoise::Ardoise(ePlacementJoueurs placementDesJoueurs, std::vector<Joueur>& joueurs, std::vector< std::vector<Pion>* > pions)
		...
		, _joueurs(joueurs)
	{
		...
		for (Cellule& currentCell : _cellules)
		{
			...
			if (y <= 10 && y >= 7)
			{
				...
				pionPtr = (_placementDesJoueurs == ePlacementJoueurs::blancs_noirs ? _pionsPtrs.at(0)->at(iiNord++) : _pionsPtrs.at(1)->at(iiNord++));
				pionPtr->init(

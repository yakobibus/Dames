...
namespace spc_dames
{
	...
	{
	public :
		Pion() : _aspect(eAspectDuPion::normal), _statut(eStatutDuPion::libre), _motif(eMotifDuPion::nonDefini), _id(0), _manoury(0) {}
		~Pion() = default;
		Pion(const Pion& p) = default;
		Pion& operator = (const Pion& p) = default;
		//
		eAspectDuPion aspect(void) const { return _aspect; }
		void init(const eMotifDuPion& motif, unsigned id, const unsigned& manoury, const eAspectDuPion& aspect = eAspectDuPion::normal, const eStatutDuPion& statut = eStatutDuPion::libre) { _motif = motif; _id = id; _manoury = manoury; _aspect = aspect; _statut = statut; }
